...
@php
    $variants = [
        ...
        'outline' => 'bg-transparent text-ink border border-gold hover:bg-ink hover:text-ivory hover:border-ink',
        'on-dark' => 'bg-transparent text-ivory border border-gold hover:bg-gold hover:text-ink',
    ];
@endphp

{{-- Opens the WhatsApp conversation directly. rel="noopener" because the link
     targets a new context; no pre-filled message is attached, since a
     scripted opening line reads as a marketing funnel rather than a law
     office. --}}
<a href="{{ config('site.whatsapp_href') }}"
   ...
   {{ $attributes->class([
    ...
    <svg class="size-[1.15em] shrink-0" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
